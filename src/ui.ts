...
import { SaveablePolicyNetwork } from './network'
...
const mountainCarCanvas = getElementById(
  'mountain-car-canvas'
...
const bestPosition = getElementById('best-position') as HTMLSpanElement
...
export async function maybeRenderDuringTraining(cartPole: MountainCar) {
 ...
 */
function onIterationEnd(iterationCount: number, totalIterations: any) {
...
let maxPositionValues: Array<any> = []
function plotSteps() {
  tfvis.render.linechart(
    stepsContainer,
    { values: maxPositionValues },
    {
 ...
 */
function renderMountainCar(cartPole: MountainCar, canvas: HTMLCanvasElement) {
  ...
  const cartY =
    canvas.height - (Math.sin(3 * cartPole.position) * 0.45 + 0.55) * scale
  ...
  testButton.addEventListener('click', async () => {
    ...
    while (!isDone) {
      ...
      tf.tidy(() => {
        ...
        const action = policyNet.model.chooseAction(
          mountainCar.getStateTensor(),
          0
        )
        logStatus(
          `Test in progress. ` +
